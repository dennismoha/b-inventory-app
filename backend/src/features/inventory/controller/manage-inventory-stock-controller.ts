...
export class ProductSummaryController {
    ...
    public async fetchLowStock(req: Request, res: Response): Promise<void> {
        ...
        const lowStock = summaries.filter(
            (p) => p.total_received - p.total_sold <= p.reorder_level
        );
        ...
        res
            ...
            .json(GetSuccessMessage(200, mapLowStock, 'Low stock products fetched successfully'));
    }
    ...
    public async fetchStock(req: Request, res: Response): Promise<void> {
        ...
        const mapStock: StockResponseItem[] = summaries.map((prod) => {
            ...
            return {
                ...
                total_cost_value_in_our_stock_price:  prod.supplierProduct.ProductPricing?.price?  (prod.total_received - prod.total_sold) * Number(prod.supplierProduct.ProductPricing?.price): 0,
                // derived
        ...
        res
            .status(StatusCodes.OK)
            .json(GetSuccessMessage(200, mapStock, 'Low stock products fetched successfully'));
    }
    ...
    public async updateStockLevel(req: Request, res: Response): Promise<void> {
        ...
        const updated = await prisma.productSummary.update({
        ...
        res
            .status(StatusCodes.OK)
            .json(GetSuccessMessage(200, mapLowStock, 'Stock level updated successfully'));
    }
}
