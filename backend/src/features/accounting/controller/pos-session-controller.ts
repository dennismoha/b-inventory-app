...
/***
 * session closere.
 * receives request from the frontend to close the session.
 * create AccoutnCollection record to link the session to the account snapshot.
 * 
 * model AccountCollection {
  collection_id String     @id @default(uuid()) @db.Uuid
  snapshot_date DateTime   @default(now())
...
get the collection_id of the ACcountCollection record.
  select all accounts with their current balance and save them in the AccountBalanceSnapshot table.
    model Account {
    ...
    current_balance         Decimal         @db.Decimal(14, 2) @default(0.00)
    ...
    // Relations
    cashRegisters   CashBookLedger[]
    AccountBalances AccountBalanceSnapshot[]
    AccountStatusLog AccountStatusLog[]
    // OpeningClosingBalance OpeningClosingBalance[]
  }

  from the above we should create a  record in the AccountBalanceSnapshot table for each account with the following details:
  - snapshot_id: uuidv4()
  - account_id: account.account_id
  - balance: account.current_balance
  - collection_id: the collection_id from the AccountCollection record created above.
...
for each account from all current_balance total it . That will be the closing balance for the session.

we go to the openingClosingBalance table and update the record with the following details:
- closing_date: Date.now(),
- closing_balance: the total closing balance for all accounts in the session.
- account_collection_id: the collection_id from the AccountCollection record created above.
...
go to the PosSession table and update the session with the following details:
- closedBy: req.currentUser!.email; // Get the user ID from the current user context
- closedAt: Date.now()
- status: 'CLOSED'
...
model PosSession {
  ...
  openedUser   User                     @relation("OpenedSessions", fields: [openedBy], references: [email])
  closedUser   User?                    @relation("ClosedSessions", fields: [closedBy], references: [email])
  balances     OpeningClosingBalance[]
  // ledgerEntries CashBookLedger[]
}

then remove req.currentUser!.posSessionId; // Remove the session ID from the current user context
